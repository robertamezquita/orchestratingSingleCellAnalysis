...
```{r eval=FALSE}
## total raw counts
counts_pbmc <- cbind(counts(pbmc3k[hvg_genes, ]), 
                     counts(pbmc4k[hvg_genes, ]))

## total normalized counts (with multibatch normalization)
logcounts_pbmc <- cbind(logcounts(pbmc3k[hvg_genes, ]), 
                        logcounts(pbmc4k[hvg_genes, ]))

sce <- SingleCellExperiment( 
    assays = list(counts = counts_pbmc, logcounts = logcounts_pbmc),  
    rowData = rowData(pbmc3k[hvg_genes, ]), # same as rowData(pbmc4k[hvg_genes, ]) 
    colData = rbind(colData(pbmc3k[hvg_genes, ]), colData(pbmc4k[hvg_genes, ])) 
) 
```
...
The naive approach simply entails visualizing the combined `sce` object post-normalization with no attempt at batch correction. Here we manually calculate the PCA on the normalized data (retrieved via `logcounts(sce)` or, similarly, via `assay(sce, "logcounts")`, and then assign the result into the `reducedDim` slot of `sce`, naming it `"PCA"`.
...
```{r eval=FALSE}
limma_corrected <- 
    limma::removeBatchEffect(logcounts(sce),batch = sce$Sample)
assay(sce, "logcounts_limma") <- limma_corrected ## add new assay
rm(limma_corrected)
```

```{r}
plotPCASCE(sce, by_exprs_values = "logcounts_limma", 
                      colour_by="Sample") + 
...
```{r eval=FALSE}
rescaled <- multiBatchNorm(pbmc3k[hvg_genes, ], pbmc4k[hvg_genes, ])
...
mnn_out <- fastMNN(pbmc3k_rescaled, pbmc4k_rescaled,
