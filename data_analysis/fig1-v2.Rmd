...
## Cleaning up SingleCell

* TRONCO is a package that does not appear to be primarily SingleCell.

```{r}
bad <- c('TRONCO')

pkg_tbl <- pkg_tbl %>%
    filter(!(Package %in% bad))
```

## Removing Microarray

For our purposes, microarray will be removed in order to focus on development in the sequencing era of Bioconductor. However, flow and mass spec will be kept in as these are single cell assays.

```{r}
## pkg_tbl <- pkg_tbl %>%
##     filter(!(Assay %in% 'Microarray'))
```
...
pkg_tbl %>%
    ...
    scale_y_log10() +
    facet_wrap(~ Assay, ncol = length(colours_terms)) +
...
```{r}
exclude <- c('Microarray', 'FlowCytometry', 'MassSpectrometry')

## Determine the most popular assay types by the latest time point (excluding non-seq tech)
tmp <- pkg_tbl %>%
    group_by(Date, Assay) %>%
    summarise(s = sum(Nb_of_distinct_IPs)) %>%    
    filter(Date == as.Date('2019-01-01')) %>%
    arrange(s) %>%
    filter(!(Assay %in% exclude))     

## Reorder factors by popularity at 2019, use distinct IPs, filter by assays
dat_tbl <- pkg_tbl %>%
    select(-Nb_of_downloads) %>%
    filter(Nb_of_distinct_IPs != 0) %>%
    filter(!(Assay %in% exclude)) %>%
    mutate(Assay = fct_relevel(Assay, tmp$Assay))

## Redo colors to be pretty
new_terms <- unique(tmp$Assay)
## colours_terms <- colorRampPalette(pal_npg()(10))(length(new_terms))
colours_terms <- pal_jama(alpha = 0.7)(length(new_terms))
names(colours_terms) <- new_terms

## Panel A: stacked area plot of distinct IPs
g1 <- dat_tbl %>%
    group_by(Date, Assay) %>%
    summarise(s = sum(Nb_of_distinct_IPs)) %>%
    arrange(Date, Assay) %>%
    ...
    mutate(cs = cumsum(s)) %>%
    mutate(cs_l = lag(cs)) %>%
    mutate(cs_l = ifelse(is.na(cs_l), 0, cs_l)) %>%
    ...
    labs(x = '', y = 'Cumulative sum of Distinct IPs')

## Panel B: number of packages
g2 <- dat_tbl %>%
    group_by(Date, Assay) %>%
    count() %>%
    arrange(Date, Assay) %>%
...
ggsave('data_analysis/fig1-v2_stacked-area-chart.pdf')

## ## Panel C: Focus on Single-Cell
## dat_tbl %>%
